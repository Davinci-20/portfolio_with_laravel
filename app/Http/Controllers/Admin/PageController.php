...
class PageController extends Controller
{
    ...
    public function dashboard(){
        $date=date('Y-m-d');
        $pj_count=Project::whereDate('created_at',$date)->count();
        // $insms=Contact::whereDate('created_at',$date)->count();
        $contact=Contact::whereDate('created_at',$date)->get();
        return view('admin.dashboard',compact('pj_count','contact'));
    }

    public function blogshow(){
            $show=Project::orderBy('id','DESC')->paginate(4);
